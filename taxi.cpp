...
void manage_login()
{
	...
	if (!strcmp(s, passward)) { //若密码正确
		cout << "Welcome to the Taxi system , manage \n\n";
		system("pause");
		system("cls");
		manage_mast(i);
	}
...
void usermenu(int index) {   // User menu
	int choice;       
	color(0);// usermenu , drivermenu wenama screen ekakata ganna thiyenawan hodai
	cout << " \n";
	cout << "\t\t************TourMate ********************** " << endl;
...
label3:
	...
	case 3:
		//help();
		color(0);
		cout << "If you have any question piease phone 123456789" << endl;
		break;                            
	case 4:
		//contactus();
		color(0);
		cout << "phone number:123456789"    << endl;
		cout << "e-mail：12345689@xjtlu.cn" << endl;
		break;
	case 5:
		//aboutus();
		color(0);
		cout << "This project is made by XJTLU student."<<endl;
...
void user_register() {       // Function for user Registraion
	...
	string regUser, regPass, regName, regTPN, regNIC, ru, rp, uname, up;
...
top1:
	...
	User.push_back({ regUser,regPass,regName,regTPN,regNIC });
	update_files();
...
void driver_register() {     // Function for driver registration
	...
	string regDriver, regPass, regName, regTPN, regCAR, regNumPlate, regRate, regLocation, availability;
...
label4:
	...
	Driver.push_back({ regDriver,regPass,regName,regTPN,regCAR,regNumPlate,regLocation,regRate,availability });
	update_files();
...
void RPFD() {
	...
	cout << "\t\t******** TourMate ************* " << endl;
	cout << "\t\t| 1. Reward                      | " << endl;
	cout << "\t\t| 2. Punishment                 | " << endl; // login screen
...
label1:
	...
	case 1:
		...
		scanf("%s %d", str, &i);
		i = Records.size();
		printf("The Reward saved successfully"); 
		break;
	case 2:
		driverlogin();
		break;
